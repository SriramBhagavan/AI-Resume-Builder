...
import { ArrowLeftIcon, ChevronLeft, ChevronRight, Download, DownloadIcon, EyeIcon, EyeOffIcon, FileText, FolderIcon, GraduationCap, Share2Icon, Sparkles, User } from 'lucide-react'
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="relative lg:col-span-5 rounded-lg overflow-hidden">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1">
              ...
              <hr
                className="absolute top-0 left-0 h-1 bg-gradient-to-r from-green-500 to-green-500 border-none transition-all duration-200"
                style={{
              ...
              <div className="space-y-6">
                ...
                {activeSection.id === 'experience' && (
                  <ExperienceForm data={resumeData.experience} onChange={(data)=>setResumeData(prev=>({...prev,
                    experience:data}))} setResumeData={setResumeData}/>
                )}
                {activeSection.id === 'education' && (
                  <EducationForm data={resumeData.education} onChange={(data)=>setResumeData(prev=>({...prev,
                    education:data}))} setResumeData={setResumeData}/>
                )}
                 {activeSection.id === 'projects' && (
                  <ProjectForm data={resumeData.project} onChange={(data)=>setResumeData(prev=>({...prev,
                    project:data}))} setResumeData={setResumeData}/>
                )}
                 {activeSection.id === 'skills' && (
                  <SkillsForm data={resumeData.skills} onChange={(data)=>setResumeData(prev=>({...prev,
                    skills:data}))} setResumeData={setResumeData}/>
                )}
              </div>
              <button onClick={()=>{toast.promise(saveResume(),{loading:'Saving...',  success: 'Resume saved successfully!',
    error: 'Failed to save resume!',})}} className='bg-gradient-to-br- from-green-100 to-green-200 ring-green-300 text-green-600 ring hover:ring-green-400
