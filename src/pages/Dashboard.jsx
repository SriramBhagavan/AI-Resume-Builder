...
pdfjsLib.GlobalWorkerOptions.workerSrc = pdfWorker;
  ...
  import { FilePenIcon, FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, UploadCloudIcon, XIcon } from 'lucide-react'
...
import { setLoading } from '../app/features/authSlice'
  ...
  const headers = { Authorization: `Bearer ${token}` }; // ✅ define headers here
...
const uploadResume = async (event) => {
  ...
  const deleteResume=async(resumeId)=>{
    ...
    useEffect(()=>{
      if(token){
        loadAllResumes()
        console.log('Fetching resumes...', token);
    ...
    return (
      <div>
        <div className='max-w-7xl mx-auto px-4 py-8'>
          <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome, sriram bhagavan</p>
